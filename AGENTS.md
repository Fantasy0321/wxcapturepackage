...
- **第三方代码**：`src/third-party/` 目录下的代码提取自微信开发者工具，腾讯控股有限公司拥有完整版权。
- **许可证**：本程序使用 GPLv2 许可证发布，分包时需要遵守 GPLv2 相关条款。
- **用途限制**：本工具仅用于**学习和教育目的**。使用风险自负。
- **版本查看方法**：打开任务管理器 -> 找到 WeChatAppEx 进程 -> 右键打开文件所在位置 -> 查找 `RadiumWMPF` 和 `extracted` 之间的数字即为 WMPF 版本号。
- **Git 忽略规则**：`release/` 目录和 `node_modules/` 被 gitignore 忽略。
